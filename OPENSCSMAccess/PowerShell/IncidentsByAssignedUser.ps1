...
$affectedUserRelClass = Get-SCSMRelationshipClass -Name "System.WorkItemAssignedToUser$" -computername $irScsmEnviroment;
...
ConvertTo-Json @($result | Select-Object @{Name = 'Id'; Expression = { $_.Values | Where-Object {$_.Type -in @('Id') } | Select-Object $_.Parent | Where-Object {$_.Type -in @('Id')} |  ForEach-Object Value}}, 
                        ...
                        @{Name = 'LastDateTimeModified'; Expression = { $_.LastModified.DateTime }}  )